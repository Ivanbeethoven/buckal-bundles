...
def arg_parse() -> Args:
    ...
    # *BUCKAL-ONLY* list of environment variable source files separated by commas
...
def main() -> None:  # noqa: C901
    ...
    for extra_env_path in args.extra_env:
        extra_env_src = Path(extra_env_path)
    ...
    # *BUCKAL-ONLY* metadata patterns
    cargo_metadata_pattern = re.compile("^cargo::?metadata=(.+?)=(.*)")
    # To support older versions, unsupported directives are assumed to be metadata keys
    ...
    for line in script_output.split("\n"):
        ...
        if cargo_metadata_match:
            key = cargo_metadata_match.group(1)
            value = cargo_metadata_match.group(2)
            cargo_link_name = env.get("CARGO_MANIFEST_LINKS")
            if cargo_link_name:
                key = f"DEP_{cargo_link_name.upper().replace('-', '_')}_{key.upper().replace('-', '_')}"
                metadata[key] = value
                continue
        cargo_kv_match = cargo_kv_pattern.match(line)
        if cargo_kv_match:
            key = cargo_kv_match.group(1)
            value = cargo_kv_match.group(2)
            cargo_link_name = env.get("CARGO_MANIFEST_LINKS")
            if cargo_link_name:
                key = f"DEP_{cargo_link_name.upper().replace('-', '_')}_{key.upper().replace('-', '_')}"
                metadata[key] = value
    ...
    # *BUCKAL-ONLY* output metadata as JSON
    json.dump(metadata, args.outenv, indent=2)
